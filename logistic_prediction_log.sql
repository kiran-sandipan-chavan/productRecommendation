...
-- (This CTE is now directly embedded into ML.PREDICT below)
, PredictionInput AS (
    SELECT
      uup.user_id,
      uup.product_id,
    ...
    LEFT JOIN
      UserInvoiceFeaturesPrediction AS uifp
    ON
...
FROM
  ML.PREDICT(MODEL `directed-asset-449105-t9.ML_DS.product_likelihood_logreg_model`, TABLE PredictionInput)
ORDER BY
  predicted_likelihood DESC;

-- Inspect predictions from Logistic Regression (showing top N and then grouping for unique user-product pairs):
SELECT
  user_id,
  product_id,
...
-- Optional: LIMIT here to inspect a few, or remove for full table
LIMIT 10;


--------------------------------------------------------------------------------
-- Generating Explanations for Logistic Regression Predictions (ML.EXPLAIN_PREDICT)
--------------------------------------------------------------------------------
-- This query will give you feature attributions for why a specific prediction was made.
-- We are directly embedding the PredictionInput CTE's query as the input to ML.EXPLAIN_PREDICT.

CREATE OR REPLACE TABLE
  `directed-asset-449105-t9.ML_DS.user_product_explanations_logreg` AS
WITH
  -- Re-define the PredictionInput CTE directly here for ML.EXPLAIN_PREDICT to consume it
  AllUsers AS (SELECT DISTINCT AccountID FROM `directed-asset-449105-t9.ML_DS.Account`),
  AllProducts AS (SELECT DISTINCT ItemId FROM `directed-asset-449105-t9.ML_DS.InvoiceItem`),
  AllPossibleUserProductPairs AS (
    SELECT u.AccountID AS user_id, p.ItemId AS product_id
    FROM AllUsers AS u CROSS JOIN AllProducts AS p
  ),
  UnseenUserProductPairs AS (
    SELECT app.user_id, app.product_id
    FROM AllPossibleUserProductPairs AS app
    LEFT JOIN `directed-asset-449105-t9.ML_DS.product_interactions_for_mf` AS pi
    ON app.user_id = pi.user_id AND app.product_id = pi.product_id
    WHERE pi.user_id IS NULL
  ),
  UserInvoiceFeaturesPrediction AS (
    SELECT AccountID, ANY_VALUE(SalesChannel) AS user_sales_channel, ANY_VALUE(UA_Device) AS user_ua_device
    FROM `directed-asset-449105-t9.ML_DS.InvoiceFact` GROUP BY AccountID
  )
SELECT
  *
FROM
  ML.EXPLAIN_PREDICT(
    MODEL `directed-asset-449105-t9.ML_DS.product_likelihood_logreg_model`,
    ( -- Begin the inner query for PredictionInput
      SELECT
        uup.user_id,
        uup.product_id,
        a.Occupation AS user_occupation,
      ...
      LEFT JOIN
        `directed-asset-449105-t9.ML_DS.InvoiceItem` AS item
        ON uup.product_id = item.ItemId
    ), -- End the inner query for PredictionInput
    STRUCT(10 as top_k_features) -- Optional: specify top N features to explain
  )
ORDER BY
  user_id, predicted_target_interaction DESC -- Corrected column name
LIMIT 10;
...
SELECT * FROM `directed-asset-449105-t9.ML_DS.user_product_explanations_logreg`;